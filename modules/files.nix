...
let
  ...
  sourceStorePath = file:
    let
      srcText = toString file.source;
      base = baseNameOf srcText;
    in
      if builtins.hasContext srcText
      then file.source
      else
        builtins.path {
          path = file.source;
          name = if hasPrefix "." base then substring 1 (-1) base else base;
        };
...

{
  ...
  config = {
    ...
    home.activation.checkFilesChanged = dag.entryBefore ["linkGeneration"] (
      ...
      '' + concatMapStrings (v: ''
        cmp --quiet "${sourceStorePath v}" "${config.home.homeDirectory}/${v.target}" \
